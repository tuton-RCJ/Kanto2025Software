...
sensor.skip_frames(time=300)  # Let the camera adjust.
...
thre_red = (25, 50, -30, 0, 20, 60)
thre_green = (0, 50 -10, 10, -10, 10)
...
while True:
    ...
    print(pred)
    for i, detection_list in enumerate(pred):
        if i == 0:
    ...
    """
    for b in img.find_blobs([thre_red],merge=True,pixels_threshold=200):
        img.draw_rectangle(b[0:4],color=(255,0,0))
    for b in img.find_blobs([thre_green],merge=True,pixels_threshold=200):
